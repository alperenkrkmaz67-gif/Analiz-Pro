// auth.js - Firebase (Auth) Entegrasyonu - HATA ÃÃZÃLMÃÅ VERSÄ°YON

// Global deÄiÅkenlerin varlÄ±ÄÄ±nÄ± garanti altÄ±na alalÄ±m
...
const Auth = {
    ...
    login: async (email, password) => {
        ...
        try {
            ...
            } catch (e) { console.log("IP alÄ±namadÄ±"); }

            localStorage.setItem('oa_current_user', JSON.stringify(userData));
            return { success: true, user: userData };
        } catch (error) {
            console.error("Login Error:", error);
            let message = "GiriÅ baÅarÄ±sÄ±z.";
            if (error.code === 'auth/invalid-credential' || error.code === 'auth/wrong-password') message = "E-posta veya Åifre hatalÄ±.";
    ...
    register: async (username, email, password, phone) => {
        ...
        try {
            ...
            return { success: true, message: "KayÄ±t baÅarÄ±lÄ±! GiriÅ yapabilirsiniz." };
        } catch (error) {
            let message = "KayÄ±t yapÄ±lamadÄ±: " + error.message;
            if (error.code === 'auth/email-already-in-use') message = "Bu e-posta zaten kullanÄ±mda.";
            return { success: false, message: message };
        }
    },
    ...
    updateUI: (user, skipRedirect = false) => {
        ...
        if (user) {
            ...
            if (!skipRedirect) {
                if (isAdminPage && user.role !== 'admin') window.location.href = '/';
    ...
    updateProfile: async (updates) => {
        ...
        if (!user) return { success: false, message: "KullanÄ±cÄ± oturumu bulunamadÄ±." };

        try {
            await _db.collection("users").doc(user.uid).update(updates);

            // Local storage gÃ¼ncelle
            ...
            return { success: false, message: "GÃ¼ncelleme baÅarÄ±sÄ±z." };
        }
    },
    ...
    updatePassword: async (currentPassword, newPassword) => {
        ...
        if (!user) return { success: false, message: "Oturum bulunamadÄ±." };
        ...
        try {
            ...
            return { success: true, message: "Åifre baÅarÄ±yla deÄiÅtirildi." };
        } catch (error) {
            console.error("Password Update Error:", error);
            if (error.code === 'auth/wrong-password') return { success: false, message: "Mevcut Åifre hatalÄ±." };
            return { success: false, message: "Åifre deÄiÅtirilemedi: " + error.message };
        }
    },
    ...
    validateAccess: (targetUrl) => {
        ...
        if (!user) return { allowed: false, reason: 'guest', message: 'Analiz iiçin giriÅ yapmalÄ±sÄ±nÄ±z.' };

        // Admin her zaman eriÅebilir
        if (user.role === 'admin') return { allowed: true };

        // VIP kontrolÃ¼: Hem rol hem de sÃ¼re kontrolÃ¼
        if (user.role === 'vip') {
            if (user.vipExpiry && new Date(user.vipExpiry) > new Date()) {
                return { allowed: true };
            } else {
                return { allowed: false, reason: 'expired', message: 'VIP sÃ¼reniz dolmuÅtur. Lütfen yenileyiniz.' };
            }
        }
    ...
    protectPage: () => {
        ...
        if (!result.allowed) {
            ...
            window.location.replace(result.reason === 'guest' ? 'login.html' : 'contact.html');
...
const Visitor = {
    init: async () => {
        ...
        // 1. Yeni ZiyaretÃ§i KontrolÃ¼
        if (!visitorId) {
            visitorId = 'v_' + Date.now() + '_' + Math.random().toString(36).substr(2, 9);
            localStorage.setItem(STORAGE_KEY, visitorId);

            // Global toplamÄ± artÄ±r
            try {
        ...
        // 2. GÃ¼nlÃ¼k Ziyaret KontrolÃ¼
        if (lastVisitDate !== today) {
            localStorage.setItem(LAST_VISIT_KEY, today);

            // GÃ¼nlÃ¼k toplamÄ± artÄ±r
            try {
...
// Global eriÅim iiçin window'a baÄla
window.Auth = Auth;
window.Visitor = Visitor;

// Sayfa yÃ¼klendiÄinde otomatik baÅlat
document.addEventListener('DOMContentLoaded', () => {
    Visitor.init(); // ZiyaretÃ§i takibini baÅlat
    ...
    document.addEventListener('click', (e) => {
        ...
        if (!result.allowed) {
            e.preventDefault();
            alert(result.message);
            window.location.href = result.reason === 'guest' ? 'login.html' : 'contact.html';
        }
    });
