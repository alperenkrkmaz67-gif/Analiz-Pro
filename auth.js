// auth.js - Firebase (Auth) Entegrasyonu - HATA ÇÖZÜLMÜÅ VERSİYON
...
const Auth = {
    ...
    updatePassword: async (currentPassword, newPassword) => {
        ...
        try {
            ...
            return { success: true, message: "Åifre başarıyla değiştirildi." };
        } catch (error) {
            console.error("Password Update Error:", error);
            if (error.code === 'auth/wrong-password') return { success: false, message: "Mevcut şifre hatalı." };
            return { success: false, message: "Åifre değiştirilemedi: " + error.message };
        }
    },
    ...
    validateAccess: (targetUrl) => {
        ...
        if (!user) return { allowed: false, reason: 'guest', message: 'Analiz iiçin giriş yapmalısınız.' };
...
// Global erişim iiçin window'a bağla
window.Auth = Auth;
